...
import type { Topology, TopologyAnalysis, Problem } from '../types/topology.js';
...
import type { SQLGenerationRequest, SQLGenerationResult, SchemaInfo, TableInfo, SimilarQueryForContext, TableContextForSQL } from '../types/sql.js';
...
interface FailoverRecommendation {
  ...
  confidence: number;
  reasoning: string;
  alternatives: Array<{ host: string; reason: string }>;
}
...
export class AIProvider {
   ...
   */
  async analyzeTopology(topology: Topology): Promise<TopologyAnalysis> {
...
Respond only with valid JSON.`;
   ...
   */
  private basicAnalysis(topology: Topology): TopologyAnalysis {
...
Respond only with valid JSON.`;
    ...
    try {
      const response = await this.generate(prompt);
      return JSON.parse(response) as FailoverRecommendation;
    } catch (error) {
   ...
   */
  private basicRecommendation(candidates: FailoverCandidate[]): FailoverRecommendation {
...
Respond only with valid JSON.`;
    ...
    try {
      const response = await this.generate(prompt);
      return JSON.parse(response) as ParsedCommand;
    } catch (error) {
   ...
   */
  private basicParseCommand(query: string): ParsedCommand {
   ...
   */
  async explainEvent(event: ClusterEvent): Promise<string> {
...
Provide a clear, concise explanation (2-3 sentences) suitable for an operations team.`;
   ...
   */
  async generateReport(topology: Topology): Promise<string> {
...
Keep it under 200 words.`;
   ...
   */
  private basicReport(topology: Topology): string {
    const lines: string[] = [
   ...
   */
  async validateSwitchover(
    check: SwitchoverCheck,
    current: string,
    target: string
  ): Promise<{ valid: boolean; warnings: string[]; advice: string }> {
...
Respond only with valid JSON.`;
    ...
    try {
      const response = await this.generate(prompt);
      const result = JSON.parse(response) as SQLGenerationResult & { confidence?: number };
      ...
      (result as any).confidence = result.confidence || 0.7;
  ...
  private buildRAGContextSection(
    similarQueries?: SimilarQueryForContext[],
    tableContexts?: TableContextForSQL[]
  ): string {
   ...
   */
  private buildSchemaContext(schema?: SchemaInfo, database?: string): string {
    if (!schema || schema.tables.length === 0) {
      return 'No database schema information available.';
    }

    const lines: string[] = [`Database: ${database || schema.database}`, '', 'Tables:'];
   ...
   */
  private isSQLSafe(sql: string, readOnly: boolean): boolean {
