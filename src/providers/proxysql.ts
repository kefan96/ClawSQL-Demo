...
export class ProxySQLProvider {
  ...
  private dataPool: Pool | null = null;
   ...
   */
  private getDataPool(database?: string): Pool {
    if (!this.dataPool) {
  ...
  async executeSQL(
    ...
    const columns = fields.map((f: FieldPacket) => f.name);
   ...
   */
  async getSchema(database?: string): Promise<{ database: string; tables: Array<{ name: string; columns: Array<{ name: string; type: string; nullable: boolean; key: string | null }> }> }> {
    const pool = this.getDataPool(database);
