...
import type { Topology, Problem, InstanceWithReplication, TopologyDiff } from '../types/topology.js';
import type { ClusterEvent, EventHandler } from '../types/events.js';
...
export class TopologyService {
  ...
  private config: TopologyServiceConfig;
  private currentTopology: Topology | null = null;
  private pollTimer: ReturnType<typeof setInterval> | null = null;
  private eventHandlers: Map<string, Set<EventHandler>> = new Map();
  private previousTopology: Topology | null = null;
   ...
   */
  async discoverCluster(): Promise<Topology> {
    ...
    const instancesWithRepl: InstanceWithReplication[] = await Promise.all(
      instances.map(async (instance): Promise<InstanceWithReplication> => {
        let replication: ReplicationStatus | null = null;
        if (instance.isReplica) {
          replication = await this.mysqlProvider.getReplicationStatus(
            instance.host,
            instance.port
          );
        }
   ...
   */
  private async captureSchemasInBackground(): Promise<void> {
    try {
   ...
   */
  getInstance(host: string, port: number = 3306): InstanceWithReplication | undefined {
  ...
  private detectProblems(
    ...
    instances: InstanceWithReplication[]
  ): Problem[] {
    ...
    for (const instance of instances) {
      if (!instance.isReplica || !instance.replication) continue;
  ...
  private diffTopology(
    oldTopo: Topology | null,
    newTopo: Topology
  ): TopologyDiff | null {
    if (!oldTopo) return null;
   ...
   */
  private emitTopologyChange(diff: TopologyDiff): void {
...
// Singleton instance
let _service: TopologyService | null = null;

export function getTopologyService(config?: TopologyServiceConfig): TopologyService {
  if (!_service && config) {
...
export function resetTopologyService(): void {
  _service = null;
}