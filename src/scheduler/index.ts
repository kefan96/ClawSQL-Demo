...
export class Scheduler {
   ...
   */
  private async runSchemaSync(): Promise<void> {
    ...
    try {
      ...
      for (const database of databases) {
        try {
          const result = await this.memoryService.detectSchemaChanges(
          ...
          if (result.detected) {
            log.info(
              { database, changes: result.changes.length },
              'Schema changes detected during sync'
            );
          }
        } catch (error) {
   ...
   */
  async triggerSchemaSync(): Promise<void> {
    await this.runSchemaSync();
  }
}
