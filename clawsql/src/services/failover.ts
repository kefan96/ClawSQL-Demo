...
import type { Instance, ReplicationStatus } from '../types/mysql.js';
import type { FailoverCandidate, SwitchoverCheck, SwitchoverResult, FailoverResult, PromoteResult, RollbackResult, ValidationResult, FailoverState, PreviousTopology } from '../types/failover.js';
import type { Topology, InstanceWithReplication } from '../types/topology.js';
...
import { getProxySQLProvider, Hostgroups, type HostgroupValue } from '../providers/proxysql.js';
...
export class FailoverService {
  ...
  private config: FailoverServiceConfig;
  private state: FailoverState;
  private previousTopologies: PreviousTopology[] = [];
   ...
   */
  async canSwitchover(): Promise<SwitchoverCheck> {
    const result: SwitchoverCheck = {
      canSwitchover: true,
      reasons: [],
      warnings: [],
    ...
    try {
      ...
      // Check for problems
      const criticalProblems = topology.problems.filter(
        p => p.severity === 'critical' || p.type === 'broken_replication'
      );
   ...
   */
  async switchover(target?: string): Promise<SwitchoverResult> {
    ...
    try {
      ...
      // Determine target
      let targetInstance: InstanceWithReplication;
      if (target) {
        ...
        const found = topology.replicas.find(
          r => r.host === bestCandidate.host && r.port === bestCandidate.port
        );
        if (!found) {
      ...
      const caughtUp = await this.mysqlProvider.waitForGTID(
        targetInstance.host,
        targetInstance.port,
        primaryGTID,
        this.config.failoverTimeout * 1000
      );
      if (!caughtUp) {
      ...
      for (const replica of topology.replicas) {
        ...
        try {
          await this.mysqlProvider.setupReplication(
            replica.host,
            replica.port,
            targetInstance.host,
            targetInstance.port
      ...
      await this.proxysqlProvider.switchWriter(
        oldPrimary.host,
        oldPrimary.port,
        targetInstance.host,
        targetInstance.port
      ...
      log.info(
        { oldPrimary: oldPrimary.host, newPrimary: targetInstance.host, duration },
  ...
  async failover(): Promise<FailoverResult> {
    ...
    try {
      ...
      try {
        ...
        if (topology.primary) {
          await this.proxysqlProvider.removeServer(
            topology.primary.host,
            topology.primary.port,
            Hostgroups.WRITER
          );
        }
        // Add new primary as writer
        await this.proxysqlProvider.addServer(
          Hostgroups.WRITER,
          newPrimary.host,
          newPrimary.port
        );
   ...
   */
  async emergencyPromote(host: string, port: number = 3306): Promise<PromoteResult> {
    try {
   ...
   */
  private async findFailoverCandidates(topology: Topology): Promise<FailoverCandidate[]> {
   ...
   */
  private savePreviousTopology(topology: Topology, reason: string): void {
...
// Singleton instance
let _service: FailoverService | null = null;

export function getFailoverService(config?: FailoverServiceConfig): FailoverService {
  if (!_service && config) {
