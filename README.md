...
```
  ...
  curl -X POST http://openclaw:3000/hooks/agent
  payload:
  ┌──────────────────────────────────────┐
  │ {                                    │
...
PostFailoverProcesses:
  → POST http://openclaw:3000/hooks/agent
  → JSON body 包含 skill, failureType, failedHost/Port, successorHost/Port, isSuccessful, isMaster 等
